...
import { Home, ClipboardList, CheckSquare, User, Settings } from 'lucide-react';
...
const defaultAccentColor = '#ff0000';
...
const InteractiveMenu: React.FC<InteractiveMenuProps> = ({ items, accentColor }) => {
  ...
  const handleItemClick = (index: number) => {
    // Locked to Work Orders - no action on click
    return;
  };
      ...
      {finalItems.map((item, index) => {
        ...
        return (
          <button
            ...
            style={{ '--lineWidth': '0px', cursor: isActive ? 'default' : 'not-allowed', opacity: isActive ? 1 : 0.5 } as React.CSSProperties}
          >
